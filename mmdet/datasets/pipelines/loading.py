...
class LoadAnnotations:
    ...
    def __init__(self,
        ...
        self.file_client_args = file_client_args.copy()
        self.file_client = None
        self.with_bert_token = with_bert_token
    ...
    def _load_text_info(self, results):
        """
        load info that used for text-based person search task:
            bert_token, bert_attention_mask, pids
        """

        results['gt_pids'] = results['ann_info']['pids'].copy()
        results['descriptions'] = results['ann_info']['descriptions'].copy()

        if self.with_bert_token:
            for k in ['bert_token', 'bert_attention_mask']:
    ...
    def __call__(self, results):
        ...
        if self.with_text:
            results = self._load_text_info(results)
        if self.with_mask:
